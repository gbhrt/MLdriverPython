...
def direct_emergency_cost(state,acc,steer,Direct,trainHP,planningState = None,step_num = 0):
    ...
    roll_flag,dev_flag = Direct.check_stability2(emergencyState.Vehicle,state.env,trainHP.max_plan_deviation,planningState.var[step_num],1.0)#0.5 if planningState.last_emergency_action_active else trainHP.stabilize_factor
...
def emergency_cost(state,acc,steer,trainHP,nets = None,planningState = None,step_num = 0,Direct = None):
    ...
    for i in range(0,trainHP.rollout_n):#n times       
        ...
        #    roll_var += trainHP.one_step_var
        step_num+=1
        S_Vehicle = actions.step(S_Vehicle,acc_command,steer_command,nets.TransNet if not trainHP.direct_stabilize else Direct ,trainHP)                   
        tmp_state_vec.append(S_Vehicle) 
        state.env[1] = lib.find_index_on_path(state.env[0],S_Vehicle.abs_pos)
        ...
        #state.env[1] = lib.find_index_on_path(state.env[0],S_Vehicle.abs_pos) 
        if not trainHP.direct_constrain:
            emergency_roll_flag,emergency_dev_flag = actions.check_stability(S_Vehicle,state.env,trainHP,roll_var = planningState.var[step_num],max_plan_roll = trainHP.max_plan_roll,max_plan_deviation = trainHP.max_plan_deviation)
        else:
            emergency_roll_flag,emergency_dev_flag = Direct.check_stability2(S_Vehicle,state.env,trainHP.max_plan_deviation,planningState.var[step_num],1.0)#0.5 if planningState.last_emergency_action_active else trainHP.stabilize_factor
...
def comp_MB_action(nets,state,acc,steer,trainHP,planningState = None, Direct = None):
    ...
    if not trainHP.direct_constrain:
        roll_flag,dev_flag = actions.check_stability(state.Vehicle,state.env,trainHP,roll_var = planningState.var[0],max_plan_roll = max_plan_roll,max_plan_deviation = max_plan_deviation)
    else:
        roll_flag,dev_flag = Direct.check_stability2(state.Vehicle,state.env,trainHP.max_plan_deviation,planningState.var[0],1.0)#0.5 if planningState.last_emergency_action_active else trainHP.stabilize_factor
    ...
    if roll_flag == 0 and dev_flag == 0:     
        ...
        if not trainHP.direct_constrain:
            ...
            roll_flag,dev_flag = Direct.check_stability2(state.Vehicle,state.env,trainHP.max_plan_deviation,planningState.var[1],trainHP.stabilize_factor)#0.5 if planningState.last_emergency_action_active else trainHP.stabilize_factor
            #roll_flag,dev_flag = 0,0#tmp
            ...
            if trainHP.emergency_action_flag:
                ...
                if trainHP.direct_stabilize:
                    if planningState.trust_T < len(StateVehicle_vec):
                        ...
                        e_cost,tmp_state_vec,tmp_a_vec = emergency_cost(emergencyState,a,s,trainHP,planningState = planningState,step_num = step_num,Direct = Direct)#2 tmp
                        ...
                        if e_cost >= trainHP.max_cost and next_acc > -1.0: 
                            ...
                            e_cost,tmp_state_vec,tmp_a_vec = emergency_cost(emergencyState,a,s,trainHP,planningState = planningState,step_num = 2,Direct = Direct)#2 tmp
