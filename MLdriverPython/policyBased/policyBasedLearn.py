...
def choose_action(epsilon,action_space_len,Pi):
   ...
   # action_space.data = [dv*x for x in range(-res,0)]+[dv*x for x in range(0,res+1)]
    ...
    for i in range(num_of_runs): #number of runs
        ...
        while not done and not stop: #until game over, or user stop
             ...
             #Pi_corrected = np.copy(Pi)
             next_Pi = net.get_Pi([pos_next,vel_next])
             ##print("next_Q: ",next_Q)
             #Pi_corrected[0][a] = r +gamma*np.max(next_Pi)
             ...
             net.update_sess([pos,vel],r,a)
             #tmp_Pi = net.get_Pi([pos_next,vel_next])
            # print("tmp_Q: ",tmp_Q)
             #W1,b1 = net.get_par()
             #print("W0: ",W1[0])
             #print("W1: ",W1[1])
