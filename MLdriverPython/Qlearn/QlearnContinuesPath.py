...
def get_reward(state,max_velocity):   
...
if __name__ == "__main__": 
    ...
    feature_points = 1 #not neccecery at the beginning also 1 is good
    ...
    res = 1
    path_name = "paths/straight_path_short.txt"    #long random path: path3.txt  #long straight path: straight_path.txt
    save_name = "policy/model1.ckpt"
    restore_name = "policy/model1.ckpt"
    ###################
    ...
    for i in range(num_of_runs): #number of runs - run end at the end of the main path and if vehicle deviation error is to big
        ...
        while not stop:#while not stoped, the loop break if reached the end or the deviation is to big
            ...
            if t - last_t > step_time:
                ...
                des_vel = pl.simulator.vehicle.velocity + action_space[a]
                if des_vel > max_velocity:
                    des_vel = max_velocity
                if not first_time:
                ...
                if main_index > max_index-1: #check if end of the episode
                    #net.save_model()
                    #time.sleep(1)
                    if i % 20 == 0:
