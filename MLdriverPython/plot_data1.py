...
def get_data(folder,names_vec):
    ...
    train_indexes = [5000*j for j in range(1,101)]
    rewards_vec = []
    fails_vec = []
    series_colors = []
    series_names = []
...
if __name__ == "__main__":
   ...
   ## names_vec.append([['REVO+F1','REVO+F2','REVO+F3','REVO+F4','REVO+F5'],['REVO+F','blue']])
   # names_vec.append([['REVO+FA1','REVO+FA2','REVO+FA3','REVO+FA4'],['REVO+FA','orange']])
    ...
    for rewards,var,color,label in zip(avg_rewards_vec,var_rewards_vec,series_colors,series_names):
        plt.plot(trains[:len(rewards)],rewards,color = color,label = label)
        plt.errorbar(trains[:len(rewards)],rewards,var,c = color)
    ...
    for fails,var,color,label in zip(avg_fails_vec,var_fails_vec,series_colors,series_names):
        plt.plot(trains[:len(fails)],fails,color = color,label = label)
        plt.errorbar(trains[:len(fails)],fails,var,c = color)
    plt.legend()
