...
if __name__ == "__main__": 
    ...
    net = DDPG_network(envData.observation_space.shape[0],envData.action_space.shape[0],envData.action_space.high[0],\
        HP.alpha_actor,HP.alpha_critic,HP.alpha_analytic_actor,HP.alpha_analytic_critic,tau = HP.tau,seed = HP.seed)  
    if HP.restore_flag:
        net.restore(HP.restore_file_path)
    #init agent on analytic planner:
    #init_nets.init_net_analytic(envData,net,HP.save_file_path,HP.restore_file_path,create_data_flag = False)
